...
const adapter = (function (JDBC, jinst, yaml, log) {
  ...
  function _loadDatabaseDrivers () {
    if (!jinst.isJvmCreated()) {
      jinst.addOption('-Xrs')
      jinst.setupClasspath([`${__dirname}/drivers/informix/ifxjdbc.jar`])
    }
  }
  ...
  function _getDatabaseAuthData () {
    ...
    const { host, port, database, user, password, server } = configs.database

    return { host, port, database, user, password, server }
  }
  ...
  function _connect () {
    ...
    const { host, port, database, user, password, server } = _getDatabaseAuthData()
    const db = new JDBC({
      drivername: 'com.informix.jdbc.IfxDriver',
      url: `jdbc:informix-sqli://${host}:${port}/${database}:INFORMIXSERVER=${server};DELIMIDENT=y;IFX_ISOLATION_LEVEL=1;`,
      maxpoolsize: 100,
      minpoolsize: 1,
      password,
      user
